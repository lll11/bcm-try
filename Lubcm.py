...
class iBCM():
    ...
    def __init__(self, S, F, FG, N, ITERATION=300, alpha=0.1,
        ...
        self.p_val = zeros((self.S, self.F), dtype=int32) # lu: s is the row number - cluster is row number; feature is column number ; len such thing is giving row number
        self.p_true_val = zeros((self.S, self.F), dtype=int32)
        #self.p_index = zeros(self.S, dtype=int32)
        #lu: try a new random initialization
        #####
        self.p_index = random.randint(self.N, size= self.S) #lu: 3.11 changed self.S to self.N
        #####
        ...
        self.phi_true = [[zeros(self.FG[f]) for f in xrange(self.F)] for S in xrange(self.S)]
        ...
        ### lu - changed initial z to all 0s instead of all random numbers
        self.z = zeros((self.F, self.N), dtype = int32) # lu: not sure of meaning here

        self.z_true = []  #array([[random.randint(self.S) for f in xrange(self.F)] for s in xrange(self.N)], dtype=int32) #lu: initial code?
        self.z_true_per_data = [] #array([random.randint(self.S) for s in xrange(self.N)], dtype=int32)  # lu: initial code?
        ...
        if self.is_saving_pickle:
            ...
            jwl_picklefile_name = self.results_folder + '/' + \
                'jwl_pickle.txt'
            self.jwl_picklefile = open(jwl_picklefile_name, 'wb')
    ...
    def _initializeVariables(self, data=[], addNoiseVar=['w', 'p', 'z']): # what is this self referring to?
        '''
        ...
        if self.DEBUGGING == True: # lu: sim mode?
        ...
        if isinstance(data, cData) and len( # lu: data = data matrix?
            ...
            if len(data.p_true_val) == 0 and 'p' in addNoiseVar:
                print 'p_true does not exist! cannot add noise to p' # lu: so in real data - p_true == 0?
                sys.exit(1)
            ...
            if len(data.p_true_index) > 0:
                self.p_true_index = array(data.p_true_index) # lu: still 0s? does not appear to be diff when changed to array
        ...
        if 'p' in addNoiseVar: # lu: p is not addNoiseVar - default? - so p index is still the all 0s? after initializing ?
            if DOHALFMIX:
                BADSTART_IND = [0, 1, self.S - 1]
                for i in BADSTART_IND: # lu: go to 0,1, and cluster -1?
                    idchoosen = random.randint(self.N) # lu: go from [0,N)
                    self.p_index[i] = idchoosen # lu: each time, it should give a different number
                    self.p_val[i] = self.x[idchoosen]
                    print 'idchosen is:' % idchoosen
            else:
                self.p_index = [random.randint(self.N) for s in xrange(self.S)] # lu: it should give a different number - a uniformly rv [0,n)
                self.p_val = [self.x[self.p_index[s]] for s in xrange(self.S)] # lu: s = 1,..S, taken correponding p_index (ith chosen), data's correponding feature
                print self.p_index
        if 'z' in addNoiseVar:
            if DOHALFMIX:
	...
	# also save the pickle to a file without x in it
        pickle.dump({'p_index': self.p_index, 'p_val': self.p_val,
                     'z': self.z, 'w': self.w, 'nd': self.nd, 'nfg': self.nfg,
                     'lamb_scalar': self.lamb_scalar, 'PEAKSIZE': self.PEAKSIZE,
                     'N': self.N, 'S': self.S, 'F': self.F,
        ...
        if self.DEBUGGING:
          ...
          '''
            ...
            a_p_est.imshow([[i if self.p_index[s] == i else 0 for i in xrange(
                self.N)] for s in xrange(self.S)], interpolation='nearest', cmap= cm.RdYlGn)  #cm.Greys_r - the color does not help - not very informative
            a_p_est.axis('off')
